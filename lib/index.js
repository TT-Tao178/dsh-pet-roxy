...
const WIDGET_PATH = '/dsh-pet-roxy/widget.js'
...
// ---------------------------------------------------------------------------
// 结构化 index 注入（webserver/index-inject）
//
// 官方 Web 组合（served 模式）渲染 index.html 时会把 rows 渲染进 HTML，之后才跑
// tapIndex；而 Electron 桌面版（static 模式）加载的是 app://app/ 下的静态
// index.html，根本不经过 host 的渲染管线，tapIndex 永远不会被调用——只有同一张
// rows 表会经 IPC 交给页面侧解释器（dsh-web-frontend 的 boot 脚本），在那里
// script-src 行被实现为「createElement('script') → append 到 head → await onload」。
//
// 所以两种模式都生效的注入必须走 rows；tapIndex 只能当旧运行时兜底。
// ---------------------------------------------------------------------------
function pushIndexInjection(ctx, row) {
  if (typeof ctx.on !== 'function') return () => {}
  const handler = (table) => {
    if (!Array.isArray(table)) return
    const dup = table.some((r) => r && r.kind === row.kind && r.src === row.src)
    if (!dup) table.push({ ...row })
  }
  try {
    const disposer = ctx.on('webserver/index-inject', handler)
    return typeof disposer === 'function' ? disposer : () => {}
  } catch (err) {
    return () => {} // 运行时没有该事件：静默退回 tapIndex 路径
  }
}
...
function apply(ctx) {
  ...
  async function fetchBalance() {
    ...
    // DeepSeek 的 /user/balance 返回 {"is_available":...,"balance_infos":[...]}，没有 code 字段，
    // 所以早先的 `json.code !== 0` 判据恒为真（undefined !== 0），余额永远失败。
    // 改为以 balance_infos 是否为数组为准 —— 假设的旧格式（带 code:0）同样满足。
    if (!res.ok || !json || !Array.isArray(json.balance_infos)) {
      const cached = balanceCache ? balanceCache.value : null
      ...
      return {
        ok: false,
        code: 'API',
        error: '余额接口返回异常: HTTP ' + res.status + ' ' + String((json && json.error && json.error.message) || (json && json.msg) || (json && json.message) || (lastErr && lastErr.message) || '（响应结构不符合预期）').slice(0, 160),
      }
    }
  ...
  function getBalance(force) {
    const cfg = mergedConfig()
    const ttl = 25000
    // force：绕过 25 秒缓存。用户点宠物、点「刷新」时要的是此刻的余额，
    // 而不是最多 25 秒前的快照。
    if (force !== true && balanceCache && Date.now() - balanceCache.at < ttl) {
  ...
  function finalizeTurn(sessionId) {
    ...
    if (agg) {
      if (agg.cost > 0) {
        ...
        lastTurn = {
          turn: agg.turn,
          // 保留 4 位小数：单轮消耗常常只有几厘钱，round 到「分」会被抹成 0.00
  ...
  function handleSessionEvent(sessionId, event) {
    try {
      ...
      // inputTokens 是「聚合 prompt tokens」，cacheReadTokens 是其中命中缓存的那部分
      // （harness 自己就是这么算未命中的：missedInputTokens = promptTokens - cacheReadTokens）。
      // 所以未命中必须减掉 cache 再走 miss 价，否则缓存部分会被按 miss 价重复计一遍，
      // 而 miss 价是 hit 价的 30 倍（1.5 vs 0.05）—— 金额会明显偏高。
      // tokens 总数同理不能再加一遍 cache。
      const missInput = Math.max(0, input - cache)
      agg.tokens += input + output + reasoning
      const model = d.message && d.message.source ? d.message.source.model : ''
      const p = priceFor(model)
      const off = isPeakTime(Math.floor(Date.now() / 1000)) ? 1 : 0
      agg.cost += (cache / 1e6) * p.hit[off] + (missInput / 1e6) * p.miss[off] + ((output + reasoning) / 1e6) * p.out[off]
      agg.lastTs = Date.now()
    } catch (err) { /* 静默 */ }
  }
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: safeHandler(async (req, res) => {
      // ?force=1 绕过 25 秒缓存 —— 界面上「实时查看余额」靠它
      const payload = await getBalance(queryOf(req).get('force') === '1')
      sendOk(res, payload)
    }),
  }))
  ...
  // 主路径：结构化注入行 —— served（官方 Web 组合）与 static（Electron 桌面版）都生效。
  disposers.push(pushIndexInjection(ctx, { kind: 'script-src', placement: 'body', src: WIDGET_PATH }))

  // 兜底：不发出 webserver/index-inject 的旧运行时仍走 tapIndex。
  // WIDGET_PATH 幂等检查保证 served 模式下 rows 与 taps 不会重复加载。
  disposers.push(ctx.webServer.tapIndex((html) => {
    if (html.indexOf(WIDGET_PATH) !== -1) return html
    const tag = `<script defer src="${WIDGET_PATH}"></script>`
...
export { name, inject, apply, isPeakTime, priceFor }
